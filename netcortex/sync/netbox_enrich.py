...
def _compute_netbox_delta(
    ...
    Returns a dict keyed by field name (``"name"`` / ``"serial"``); each
    value is ``{"intent": <netbox>, "current": <observed>}``.  Empty
    dict means "no recordable delta".

    Per the NetCortex design philosophy, NetCortex is authoritative
    for current state; NetBox is intent.  We never mutate the
    observed values to match NetBox here — we just surface the gap
    so a future reconciliation UI can flag it to the operator.
    Inputs are pre-trimmed strings; the caller is responsible for
    ...
    if netbox_name and current_name and nb_name_norm != cur_name_norm:
        delta["name"] = {"intent": netbox_name, "current": current_name}
    ...
    if nb_serial_norm and cur_serial_norm and nb_serial_norm != cur_serial_norm:
        delta["serial"] = {"intent": nb_serial_norm, "current": cur_serial_norm}
...
async def enrich_devices_from_netbox(
    ...
    if not nb_by_serial and not nb_by_name:
        ...
        return {"enriched": 0, "duplicates_marked": 0}
    ...
    for _nb_id, (nb_info, graph_nodes) in nb_id_to_nodes.items():
        ...
        if delta_fields:
            ...
            delta_payload = _json.dumps(delta_fields, sort_keys=True)
            deltas_recorded += 1
        else:
        ...
        for node in ranked:
            ...
            updates.append({
                ...
                "netbox_name":       nb_info.get("netbox_name", ""),
    ...
    for rec in records:
        ...
        absent_updates.append({
            "node_id":      nid,
            "netbox_delta": '{"status": "absent_in_netbox"}',
        })
        absent_in_netbox += 1
